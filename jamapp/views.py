...
def jsreg(request):
    ...
    usertype='jobseeker'
    js=JobSeeker(name=name,gender=gender,address=address,contactno=contactno,
                 emailaddress=emailaddress,dob=bob,qualification=qualification,
                 experience=experience,keyskills=keyskills,regdate=regdate)
...
def empchangepwd(request):
    ...
    try:
        ...
        msg=msg+'oldpassword is not match'
        return render(request,'empchangepassword.html',{'msg':msg})
def jobhome(request):
...
def jobchangepwd(request):
    ...
    try:
        ...
        Login.objects.filter(userid).update(password=newpassword)
...
def appliedjobs(request,id):
    ...
    aj=AppliedJobs(empemailaddress=empemailaddress,dob=dob,jobtitle=jobtitle,post=post,name=name,gender=gender,address=address,contactno=contactno,emailaddress=emailaddress,qualification=qualification,experience=experience,keyskills=keyskills,applieddate=applieddate)
    aj.save()
    return redirect('jobhome')
